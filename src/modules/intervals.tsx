...
export const scoreIntervals = (selectedIntervals: [number, number][], questionInterval: string) => {
  const lowestNote = selectedIntervals.reduce((lowest, current) => 
    current[1] > lowest[1] ? current : lowest
  );
  const highestNote = selectedIntervals.find(interval => interval !== lowestNote)!;
  ...
  const halfStepsBetweenNotes = fretDifference + (stringDifference * 5); // this doesn't account for the B string yet, ffs
  const thisInterval = (intervals.halfStepIntervals as THalfStepIntervals)[halfStepsBetweenNotes.toString()];
